...
Important note, if you wish to reproduce the figures yourself or verify the source code, it can all be found in the Final Project folder of \cite{Constantineau_MATH_680_Code}. hello


\section{PCA} \label{sec: PCA}
In this section, we present the original PCA algorithm. Suppose you have a large dataset of $N \in \N^+$ items that are characterized by $p \in \N^+$ features. You can therefore represent every item by a vector $x_n \in \R^p$. If $p$ is small ($\leq 3$), you could simply plot your data in $\R^p$ and visualize any emerging patterns. The problem arrises when $p$ is large. In this case, it can be very hard to get an understanding of what is going on.
...
vv^Tu.
\]
Hence $g \circ f$ projects of our original data on the linear subspace spanned by the set of vectors $b_1, \dots, b_d$. Note that this subspace has dimension $\le d$ and the equality is achieved if and only if the vectors are linearly independent. We assume an even stronger condition, that the set of vectors is orthonormal. Note that this assumption is not restrictive as given a basis for a $d$-dimensional vector space, you can always obtain an orthonormal basis for the same set. Going back to the minimization problem and plugging in our new representation of $f, g$ we obtain
\[
    \frac{1}{N} \sum_{n = 1}^{N} \norm{x_n - BB^Tx_n}
\]
...
Recalling that the data was originally in $\R^{10101}$, PCA gives us a much clearly idea of what is going on. We see that in the case $d=2$, PCA is able to make apparent a pattern in the genome of each population. When $d=3$, we see that the third principal component mostly captures differences in sex.
...
\section{PCA+} \label{sec: PCA+}
In this section, we introduce PCA+, a contrastive appraoch to PCA. This will allow these PCA+ to correctly filter out background noise and only focus on the ``core" structure of the problem. The method presented will be taken from section~3 of \cite{wu2025pcauniformityinducesrobustness}. For this section, we assume we have paired data $\{x_n, x_n^+\}_{n = 1}^{N} \subset \R^p$ which can be decomposed into a shared signal, an independent background, and noise:
\[
...
\]
where each $\nu_{A,i}, \nu_{B,j}$ are unit vectors representing the principal direction with variance $\lambda_{A,i}, \lambda_{B,j}$, respectively. The idea behind the method is to contrast pairs $(x_n,x_n^+)$ that share the same signal $A w_n$ but differ in background and noise in order to isolate the shared signal and obtain some low dimensional information related to it. \cite{wu2025pcauniformityinducesrobustness} further imposes 3 conditions on the data, those being
\begin{enumerate}
...
\end{enumerate}
As \cite{wu2025pcauniformityinducesrobustness} mentions, these assumptions are there to simplify the analysis of the problem but the method remains robust to violations. We now consider a contrastive loss composed of an alignment term and a uniformity term for an encoder $f: \R^p \to \R^d$
\[
\mathcal{L}_{constrastive} = - \mathbb{E}_{(x,x^+)} \left[f(x)^Tf(x^+)\right] + \tau \norm{\mathbb{E}_x\left[f(x)f(x)^T\right] - I_d}_F,
\]
where $\tau > 0$ is a parameter controlling the strength of uniformity. We focus on the case where $\tau = 0$, which means our method only enforces alignment of positive pairs and disregards uniformty. As in PCA, we look for a linear encoder that minimizes the alignment loss. This leads to the minimization problem
\[
\min -\frac{1}{N} \sum_{n = 1}^{N} \left(V^T x_n\right)^T \left(V^T x_N^+\right) \iff \max tr\left(\frac{1}{N} V^T X^T X^+ V\right),
\]
where as before $V \in \R^{p \times d}$ is an orthonormal matrix and $X, X^+ \in \R^{N \times p}$ are matrices whose rows are the data. We do not know that $X^T X^+$ is symmetric, so we artifically construct a symmetric matrix
\[
S^+ = \frac{1}{2N}\left(X^T X^+ + {X^+}^T X\right),
\]
...
\section{PCA++}
In this section, we present a more powerful version of PCA+ better handled to deal with very high strength background noise. This method will be taken from section~4 of \cite{wu2025pcauniformityinducesrobustness}. The key idea is to not ignore the uniformity term from contrastive loss. Instead, we impose that the projected features are uniformly distributed while minimzing the alignment loss. We assume the same setup of paired data as PCA+ and maximize
\[
...
\]
We refer to this as truncated PCA++. By construction, $S_s$ is well conditioned so there is no issue. This corresponds to enforcing uniformity only on the part of $S$ which has the largest variation and therefore is less likely to be due to noise.

When analysing the error of PCA++, there are 2 different regimes \cite{wu2025pcauniformityinducesrobustness} considered. The first being that $\lambda_{A,i}, \lambda_{B_j} \ge c = \frac{p}{N}$, which is reffered to as the detectable spike regime. This means that the variance is large enough to be detectable. The second is the growing spike regime, where $N,p, \lambda_{A,i} \lambda_{B,j} \to \infty$ in such a way that
\[
...
\[
1 - \frac{1 - c \lambda_{A,d}^{-2}}{1 + c\lambda_{A,d}^{-1}},
\]
...
\centering
    ...
    \caption{\textbf{Left:} Comparing the performance of untruncated PCA++ and truncated PCA++ ($s=2$) as we vary the aspect ratio. \textbf{Right:} Comparing the performance of different truncations of PCA++ as we vary the aspect ratio.}
\label{fig: pcapp_truncations}
\end{figure}

We see from the result that truncation has a massive effect on the accuracy of PCA++ as the aspect ratio increases. At around an aspect ratio of $0.5$, untruncated PCA++ diverges completely, while truncated PCA++ deteriorates steadily. From the right plot, we see that a sharper truncation was best, with $s=2$ and $s=0.1N$ being very close to one another. The next figure we recreate is figure~3, which shows that truncated PCA++ ($s=10$) accurately follows the predicted error bounds in the detectable and growing spike regimes.
...
\section{Conclusion}
In this end of semester project, we have covered one of the most important tools in data analysis and its key variants. In addition, we have reproduced the main figures present in \cite{wu2025pcauniformityinducesrobustness} which showcase the incredible performance of PCA++ over regular PCA and PCA+. In doing this, I have truly solidified my understanding of PCA as a idea, and as an algorithm.
